...
export function aplicarEfeitosStatus(habilidade, alvo, atacante) {
  ...
  habilidade.efeitos_status.forEach(efeitoNome => {
    ...
    if (!efeitoInfo) {
      console.warn(`Efeito desconhecido: ${efeitoNome}`);
      return;
    }
...
export function inicializarBatalha(avatarJogador, avatarInimigo, dificuldade = 'normal') {
  // Aplicar penalidades de exaustão ao jogador
  const exaustaoJogador = avatarJogador.exaustao || 0;
  const nivelExaustao = getNivelExaustao(exaustaoJogador);
  const statsJogadorComPenalidade = aplicarPenalidadesExaustao({
  ...
  console.log('Exaustão do jogador:', {
    ...
    statsComPenalidade: statsJogadorComPenalidade,
  ...
  // Usar stats com penalidade para o jogador
  const jogadorComPenalidade = {
    ...avatarJogador,
    ...statsJogadorComPenalidade
  };

  const hpJogador = calcularHPMaximo(jogadorComPenalidade);
